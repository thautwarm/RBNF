...
def def_parser_maker(lang: dict) -> PAtom.Named:
    def def_parser(cls: Type[edsl.Parser]):
        def structure_getter():
            return optimize(cls.bnf())
        lang['.lazy_def_parsers'].append((cls.__name__, structure_getter))

        return N(cls.__name__)
    return def_parser

def def_lexer_maker(lang: dict) -> PAtom.Named:
    def def_lexer(cls: Type[edsl.Lexer]):
